...
/**
 * Created by maple on 2019/11/11 14:48
 */
public class ExcelUtils {
    ...
    public static void initExcel(String fileName, String tabName, String[] colName) {
        format();
        ...
        try {
            ...
            WritableSheet sheet = workbook.createSheet(tabName, 0);
            //创建标题栏
...
//				sheet.mergeCells()
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ArrayList<String> list = (ArrayList<String>) obj2Strs(objList.get(j));
                    for (int i = 0; i < list.size(); i++) {
                        sheet.addCell(new Label(i, j + 1, list.get(i), arial12format));
                        if (i == 4) {
                            sheet.setColumnView(i, list.get(i).length() + 12); //设置列宽
                        } else {
                            sheet.setColumnView(i, list.get(i).length() + 4); //设置列宽
                        }
                    }
...
//				Toast.makeText(c, "导出到手机存储中文件夹Record成功", Toast.LENGTH_SHORT).show();
    ...
    public static List<Person> readExcel(File f) {
        ...
        try {
            ...
            for (int i = 1; i < sheet.getRows(); i++) {
                ...
                tc.setPhoneNumber(cell.getContents());
    ...
    private static <T> List<String> obj2Strs(T p1){
        Person p = (Person) p1;
        List<String> strs= new ArrayList<>();
        strs.add(p.getName());
        strs.add(p.getPhoneNumber());
        strs.add(p.getMac());
        strs.add(p.getNickName());
        strs.add(p.getSex());
        strs.add(p.getArea());
        strs.add(p.getSign());
        return strs;
